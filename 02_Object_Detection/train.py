...
class DetectionTrainer:
    ...
    def _get_optimizer(self):
        ...
        if opt_name == 'sgd':
            return optim.SGD(params, momentum=momentum)
        elif opt_name == 'adam':
    ...
    def train_epoch(self, epoch: int) -> Dict:
        ...
        for batch_idx, (images, targets) in enumerate(pbar):
            ...
            pbar.set_postfix({
                ...
                'Cls': f'{loss_classifier.avg:.4f}',
                'Box': f'{loss_box_reg.avg:.4f}'
            })
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.config['epochs']):
            ...
            if epoch % self.config.get('val_frequency', 1) == 0:
                ...
                print(f"Val mAP@0.5: {val_metrics.get('map_50', 0):.4f}")
                print(f"Val mAP@0.5:0.95: {val_metrics.get('map', 0):.4f}")
                
                # Save checkpoint
                is_best = val_metrics.get('map', 0) > self.best_map
                if is_best:
...
def main():
    ...
    # Set defaults
    config.setdefault('num_classes', 21)  # VOC has 20 classes + background
